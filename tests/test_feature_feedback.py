...
def test_groups_by_role(mock_load):
    ...
    booster.feature_name.return_value = [
        "rsi_14", "funding_rate", "fear_greed_value", "btc_correlation",
    ]
    ...
    result = build_feature_feedback_addon(top_n=4)
...
def test_addon_format(mock_load):
    """addon 文本格式正确"""
    booster = MagicMock()
    booster.feature_name.return_value = ["rsi_14", "macd_hist"]
    booster.feature_importance.return_value = [100.0, 50.0]
    mock_load.return_value = (booster, "v1")

    from cryptobot.ml.feature_feedback import build_feature_feedback_addon

    result = build_feature_feedback_addon(top_n=2)
    addon = result["technical"]
    assert "### ML 特征重要性提示" in addon
    assert "rsi_14" in addon
...
def test_unmapped_defaults_to_technical(mock_load):
    ...
    result = build_feature_feedback_addon(top_n=1)
    assert "technical" in result
...
def test_top_n_limits_features(mock_load):
    ...
    booster = MagicMock()
    booster.feature_name.return_value = [
        "rsi_14", "macd_hist", "bb_width", "volume_ratio", "atr_pct",
    ]
    ...
    result = build_feature_feedback_addon(top_n=2)
    # 只取 top 2: rsi_14 (100) + macd_hist (80), 都归 technical
    addon = result["technical"]
    assert "rsi_14" in addon
    assert "macd_hist" in addon
    assert "bb_width" not in addon
