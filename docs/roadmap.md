...
> 版本: 2026-02-23 | 1220 tests passed | 20,000+ LOC | Lint clean
...
| 模块 | 文件 | 职责 |
|------|------|------|
| **仓位计算** | `risk/position_sizer.py` | Kelly 5级 fallback + 相关性折算 + 波动率自适应杠杆 |
| **爆仓距离** | `risk/liquidation_calc.py` | 爆仓价格计算 + 预警分级 |
| **跨币种相关性** | `risk/correlation.py` | Pearson 相关性矩阵 + 高相关同向限仓 + N_eff 折算 |
| **执行优化** | `risk/execution_optimizer.py` | 最优执行窗口 + 资金费率调度 + 滑点估算 |
...
| **绩效分析** | `journal/analytics.py` | 胜率/盈亏比/置信度校准/分析师准确率/按币种统计 (支持 bootstrap CI) |
| **Edge 仪表盘** | `journal/edge.py` | 期望值/Edge Ratio/SQN/R分布/Regime分组/衰减检测 |
...
| **历史回放** | `backtest/historical_replay.py` | 纯技术面 LLM 批量生成信号：分页K线下载→逐日快照→并行LLM→交易模拟（按配置隔离进度） |
| **Bootstrap CI** | `backtest/bootstrap.py` | Percentile bootstrap 置信区间 (纯 Python)，mean/median/win_rate/sharpe/profit_factor |
| **回放对比器** | `backtest/replay_comparator.py` | 多周期回放对比：Sharpe/胜率 CV + 稳定性评级(A/B/C/D) |
| **过拟合检测** | `evolution/overfit_detector.py` | 修改频率+绩效趋势+规则稳定性 → 过拟合评分(0-100) |
| **特征提取** | `features/extractors.py` | 7 个提取器：技术/多TF/链上/情绪/订单簿/宏观/相关性 |
| **特征管道** | `features/pipeline.py` | FeatureVector/FeatureMatrix + z_score/min_max 标准化 |
...
- [x] **12.1 置信度分层分析** — 回放分析器实现置信度分桶统计 + 最优阈值分析
- [x] **12.2 方向偏差诊断** — 做空偏差根因分析 + 回放 Prompt 缺失 regime addon 诊断
- [x] **12.3 回撤优化** — 日亏损熔断 5%→3%、回撤感知动态杠杆（三档缩放因子）、回放 Prompt 对齐
...
| 8.5 | ~~**Edge 仪表盘**~~ | ✅ CLI `journal edge` + Dashboard `/edge` 页面: 期望值/SQN/R分布/Regime分组/衰减检测 | 中 |
...
**180 天回放结果** (2026-02-22):

| 指标 | 值 | 说明 |
|------|-----|------|
| 总交易数 | 496 笔 | 平均 3.2 笔/天 |
| 胜率 | 46.0% | |
| 盈亏比 (PF) | 1.40 | 每亏 $1 赚 $1.40 |
| Sharpe | 4.01 | |
| Sortino | 8.15 | |
| 最大回撤 | 91.7% | ⚠️ 实盘不可接受 |
| 总收益 | +3174.7% | 初始 $10K |
...
**180d 关键发现 — 多空方向严重失衡**:
- 做空: 296 笔, 胜率 49.7%, +$17,370 ← **唯一利润来源**
- 做多: 200 笔, 胜率 40.5%, -$1,873 ← **在亏钱**
- 震荡月 (8/9/12月): 198 笔, 胜率 40.9%, **总计 -$1,012**
- 趋势月 (10/11/1/2月): 298 笔, 胜率 49.3%, **总计 +$16,509**
- 置信度 50-59: 83 笔, 胜率 36.1%, -$875 ← **应过滤**
- 置信度 80+: 10 笔, 胜率 80.0%, +$2,746 ← **高度可靠**
- **结论: 策略在趋势市表现远优于震荡市，需引入震荡市专用策略**
...
| 10.3 | ~~**置信区间替代点估计**~~ | ✅ Percentile bootstrap CI (5000 resamples) — win_rate/avg_pnl/sharpe/profit_factor 均附 95% CI | 中 |
| 10.4 | ~~**过拟合检测**~~ | ✅ 修改频率+绩效趋势+规则稳定性评分(0-100) + 每周一自动检查 cron | 中 |
...
| 11.9 | ~~**动态仓位优化**~~ | ✅ Kelly 5级 fallback(币种+方向→币种→方向→全局→默认) + 相关性折算 + ATR 波动率自适应杠杆 | 中 |
| 11.10 | ~~**跨币种相关性风控**~~ | ✅ Pearson 相关性矩阵(纯Python) + 高相关同向限仓 + N_eff折算 + 风控节点集成 | 中 |
| 11.11 | **Regime 概率模型** | 用 HMM (Hidden Markov Model) 替代规则式 regime 检测，输出各状态概率而非硬分类 | 高 |
| 11.12 | ~~**交易成本优化**~~ | ✅ 最优执行窗口 + 资金费率调度(00/08/16 UTC) + 大单滑点估算 + execution_hint 注入信号 | 中 |
...
| ID | 任务 | 说明 | 状态 | 复杂度 |
|----|------|------|------|--------|
| 12.1 | ~~**置信度分层分析**~~ | ✅ 回放分析器实现置信度分桶统计 + 最优阈值分析 | 已完成 | 低 |
| 12.2 | ~~**方向偏差诊断**~~ | ✅ 做空偏差根因分析 + 回放 Prompt 缺失 regime addon 诊断 | 已完成 | 中 |
| 12.3 | ~~**回撤优化**~~ | ✅ 日亏损熔断 5%→3% (收益+337%)、回撤感知动态杠杆(三档缩放)、回放 Prompt 对齐 | 已完成 | 中 |
| 12.4 | ~~**多周期回放验证**~~ | ✅ 进度文件按配置隔离 + `--preset 90d/180d/365d` + `replay-compare` 对比命令 + 稳定性评级 | 已完成 | 中 |
| 12.5 | **启动 VPS daemon 积累实盘信号** | 跑真实 AI 工作流 (含链上/情绪/新闻)，积累 30 天+ 归档信号 | **进行中** (2/22启动) | 低 |
| 12.6 | **实盘信号 vs 回放信号对比** | daemon 积累足够信号后，用 `backtest compare` 对比全数据源 AI vs 纯技术面回放 | 等数据 | 中 |
| 12.7 | **按币种差异化策略** | XRP 60% 胜率 +3.9%/笔 vs BNB 40% +0.8% — 考虑按币种调整杠杆/仓位/过滤 | 等数据 | 中 |

### P13 — 多策略适应：从"只赚趋势"到"全天候"

> **核心问题**: 180d 回放证实系统在震荡市亏钱（198 笔, -$1,012），趋势市赚大钱（298 笔, +$16,509）。加密市场 ~70% 时间在震荡，单一趋势策略长期不可持续。需引入震荡市专用策略 + 自适应切换机制。

#### Phase 1: Regime 检测增强 + 信号质量过滤（立即可做）

| ID | 任务 | 说明 | 复杂度 |
|----|------|------|--------|
| 13.1 | **Hurst 指数增强 Regime 检测** | 在现有 ADX+EMA regime 基础上叠加 Hurst 指数: H>0.5 趋势、H<0.5 均值回归、H≈0.5 随机游走。纯 Python 实现，无额外依赖。输出概率而非硬分类，融入 `indicators/regime.py` | 中 |
| 13.2 | **低置信度信号过滤** | 180d 数据: 置信度 50-59 的 83 笔信号胜率仅 36%，总计 -$875。在 risk_review 节点硬性过滤置信度 < 60 的信号（或 regime=ranging 时 < 65） | 低 |
| 13.3 | **做多信号加严** | 180d 数据: 做多 200 笔总计 -$1,873。在 risk_review 中对 long 信号提高置信度门槛至 65+（short 维持现有阈值），或震荡市禁止做多 | 低 |
| 13.4 | **MFE 自适应止盈** | 180d 数据: 18.7% 亏损单曾浮盈 >2% 后反转。当 MFE 达到 2×ATR 后，止损上移到入场价（保本）；之后每 1×ATR 收紧止损。修改 `AgentSignalStrategy.py` | 中 |

#### Phase 2: 震荡市策略引入（核心新增）

| ID | 任务 | 说明 | 复杂度 |
|----|------|------|--------|
| 13.5 | **布林带均值回归策略** | 价格触碰下轨(2σ)+RSI<30 做多、触碰上轨+RSI>70 做空，目标回归中轨(20MA)。仅在 regime=ranging 时启用。复用现有 `indicators/calculator.py` 已有的布林带+RSI 计算 | 中 |
| 13.6 | **Regime 策略路由器** | 在 trade 节点前增加策略路由: trending → 现有 AI 趋势策略; ranging → 均值回归策略; volatile → 观望/降仓。路由逻辑集中在新模块 `workflow/strategy_router.py` | 高 |
| 13.7 | **震荡市参数优化** | regime=ranging 时: 杠杆上限 2x（现 3x）、止损收紧到 2%（现 3-5%）、持仓时间上限 24h（现无限制）、信号数量限制每天最多 2 笔 | 低 |

#### Phase 3: 高级策略（依赖 Phase 2）

| ID | 任务 | 说明 | 复杂度 |
|----|------|------|--------|
| 13.8 | **资金费率套利** | 现货做多+永续做空，赚取正资金费率（8h 一次，2025 年均 0.015%/8h，年化 ~19%）。作为 regime=ranging 时的稳定收入源。复用 `data/onchain.py` 资金费率 + `risk/execution_optimizer.py` 费率调度 | 高 |
| 13.9 | **网格交易模块** | 在支撑/阻力之间设定等距买卖网格，适合长期震荡区间。预设上下边界+网格密度，自动低买高卖。需要 Freqtrade 策略层配合或独立执行模块 | 高 |
| 13.10 | **多策略集成权重** | 按 regime 动态分配策略权重: 趋势市 → 80% AI趋势 + 20% 网格; 震荡市 → 20% AI趋势 + 50% 均值回归 + 30% 网格; 高波动 → 观望为主。权重基于各策略历史 Sharpe 自适应 | 高 |
| 13.11 | **月度亏损熔断强化** | 180d 数据: 8-10 月连亏 3 个月(-110%)。引入月度连亏熔断: 连续 2 月亏损后自动降仓 50%+暂停做多; 连续 3 月亏损暂停交易 7 天 | 中 |

### 未完成任务时间规划

> 更新于 2026-02-23。分为"立即可做"、"需等数据"和"长期"三类。

#### A. 立即可做（P13 Phase 1 + 已有前置）— 8 项 ✅ 全部完成 (2026-02-23)

| 优先级 | ID | 任务 | 复杂度 | 状态 |
|--------|-----|------|--------|------|
| **P0** | 13.1 | Hurst 指数增强 Regime 检测 | 中 | ✅ `indicators/hurst.py` + regime.py 加权投票 |
| **P0** | 13.2 | 低置信度信号过滤 (<60) | 低 | ✅ risk.py 绝对下限 60/65(ranging) |
| **P0** | 13.3 | 做多信号加严 (≥65) | 低 | ✅ risk.py 做多≥65 + ranging 禁止做多 |
| **P0** | 13.4 | MFE 自适应止盈 | 中 | ✅ trade_simulator + AgentSignalStrategy 2×ATR 保本 |
| **P1** | 13.5 | 布林带均值回归策略 | 中 | ✅ `strategy/mean_reversion.py` BB+RSI 反转 |
| **P1** | 13.6 | Regime 策略路由器 | 高 | ✅ `workflow/strategy_router.py` 三策略路由 |
| **P1** | 13.7 | 震荡市参数优化 | 低 | ✅ ranging: 杠杆2x/日限2笔/止损2%/24h持仓 |
| **P1** | 8.2 | Walk-forward 验证 | 高 | ✅ `backtest/walk_forward.py` 60d/30d/30d 滚动 |

#### B. 需等前置完成 — 15 项

| 优先级 | ID | 任务 | 复杂度 | 等什么 | 最早可做 |
|--------|-----|------|--------|--------|----------|
| P1 | 10.5 | Regime 感知评估 | 中 | 13.1 Hurst regime | 13.1 完成后 |
| P2 | 13.8 | 资金费率套利 | 高 | 13.6 策略路由器 | 13.6 后 |
| P2 | 13.9 | 网格交易模块 | 高 | 13.6 策略路由器 | 13.6 后 |
| P2 | 13.10 | 多策略集成权重 | 高 | 13.5+13.6+13.8/13.9 | Phase 2-3 后 |
| P2 | 13.11 | 月度亏损熔断强化 | 中 | 无硬依赖，但需回放验证 | 随时 |
| P2 | 10.2 | A/B 测试增强 | 中 | VPS 实盘跑 14 天+ | ~3月8日后 |
| P2 | 12.6 | 实盘 vs 回放对比 | 中 | 12.5 VPS daemon 30 天 | ~3月24日后 |
| P2 | 12.7 | 按币种差异化策略 | 中 | 回放数据已够 | 随时 |
| P3 | 9.2 | 信号评分模型 (LightGBM) | 高 | 9.1 特征管道 + 标注 | 随时 |
| P3 | 9.3 | ML 信号过滤层 | 中 | 9.2 模型训练完成 | 9.2 后 |
| P3 | 9.4 | 特征重要性反馈 | 低 | 9.2 模型训练完成 | 9.2 后 |
| P3 | 9.5 | 模型定期重训 | 中 | 9.2 运行稳定后 | 9.2 后 |
| P4 | 11.8 | 多因子相关性分析 | 高 | 特征矩阵+长周期数据 | 9.1 后 |
| P4 | 11.11 | Regime 概率模型 (HMM) | 高 | 足够 regime 历史 | 长期 |
| P4 | 11.11b | Hurst+HMM 融合 Regime | 高 | 13.1 + 11.11 | 长期 |

#### C. 时间线总览

```
2月23日 ─── 今天 ─────────────────────────────────────────────────
  │  12.5 VPS daemon 运行中（后台持续积累）
  │  365d 回放运行中 (VPS, 预计明早完成)
  │
  ├── 本周 (2/23 - 2/28): P13 Phase 1 — 快速修复
  │   ├── 13.1 Hurst 指数增强 Regime          ← 核心改进
  │   ├── 13.2 低置信度过滤 (<60)             ← 几行代码
  │   ├── 13.3 做多信号加严 (≥65)             ← 几行代码
  │   └── 13.4 MFE 自适应止盈                 ← Freqtrade 策略改动
  │
  ├── 第 2 周 (3/1 - 3/7): P13 Phase 2 — 震荡市策略
  │   ├── 13.5 布林带均值回归策略              ← 新策略核心
  │   ├── 13.6 Regime 策略路由器              ← 架构改动
  │   ├── 13.7 震荡市参数优化                 ← 配合 13.5
  │   └── 8.2  Walk-forward 验证             ← 180/365d 数据就绪
  │
  ├── 第 3 周 (3/8 - 3/14): 验证 + 评估
  │   ├── 10.5 Regime 感知评估                ← 新 Hurst regime 验证
  │   ├── 10.2 A/B 测试增强                   ← VPS 跑满 14 天
  │   ├── 12.7 按币种差异化策略               ← 回放数据已足够
  │   └── 13.11 月度亏损熔断强化              ← 连亏保护
  │
  ├── 第 4-5 周 (3/15 - 3/28): P13 Phase 3 — 高级策略
  │   ├── 13.8 资金费率套利                   ← 稳定收入流
  │   ├── 13.9 网格交易模块                   ← 震荡市利器
  │   └── 13.10 多策略集成权重                ← 全天候系统
  │
3月24日 ─── VPS 满 30 天 ────────────────────────────────────────
  │
  ├── 第 6 周 (3/29 - 4/4): 实盘验证 + ML
  │   ├── 12.6 实盘 vs 回放对比               ← VPS 数据就绪
  │   └── 9.2  信号评分模型 (LightGBM)        ← 特征+标注就绪
  │
  ├── 第 7-8 周 (4/5 - 4/18): ML 集成
  │   ├── 9.3 ML 信号过滤层
  │   ├── 9.4 特征重要性反馈
  │   └── 9.5 模型定期重训
  │
  ├── 第 9-10 周 (4/19 - 5/2): 高级量化
  │   └── 11.8 多因子相关性分析
  │
  └── 长期
