...
> 版本: 2026-02-22 | 1125 tests passed | 19,000+ LOC | Lint clean
...
> 更新于 2026-02-22。分为"立即可做"和"需等数据"两类。

#### A. 立即可做（不依赖数据积累）— 8 项

| 优先级 | ID | 任务 | 复杂度 | 前置条件 | 建议时间 |
|--------|-----|------|--------|----------|----------|
| ~~P0~~ | ~~12.4~~ | ~~多周期回放验证 (180/365天)~~ | ~~中~~ | ✅ 已完成 | ✅ |
| ~~P1~~ | ~~10.3~~ | ~~置信区间替代点估计 (bootstrap)~~ | ~~中~~ | ✅ 已完成 | ✅ |
| ~~P1~~ | ~~8.5~~ | ~~Edge 仪表盘 (CLI + Dashboard)~~ | ~~中~~ | ✅ 已完成 | ✅ |
| ~~P2~~ | ~~10.4~~ | ~~过拟合检测 (策略修改频率监控)~~ | ~~中~~ | ✅ 已完成 | ✅ |
| ~~P2~~ | ~~9.1~~ | ~~特征工程管道 (16数据源→特征矩阵)~~ | ~~高~~ | ✅ 已完成 | ✅ |
| ~~P3~~ | ~~11.10~~ | ~~跨币种相关性风控~~ | ~~中~~ | ✅ 已完成 | ✅ |
| ~~P3~~ | ~~11.9~~ | ~~动态仓位优化 (Kelly 实时更新)~~ | ~~中~~ | ✅ 已完成 | ✅ |
| ~~P3~~ | ~~11.12~~ | ~~交易成本优化 (最优执行时间)~~ | ~~中~~ | ✅ 已完成 | ✅ |

#### B. 需等数据/前置任务完成 — 11 项

| 优先级 | ID | 任务 | 复杂度 | 等什么 | 最早可做 |
|--------|-----|------|--------|--------|----------|
| P1 | 8.2 | Walk-forward 验证 | **高** | 12.4 多周期回放数据 | 12.4 完成后 |
| P1 | 10.5 | Regime 感知评估 | 中 | 各 regime 下 ≥50 笔样本 | 12.4 完成后 |
| P2 | 10.2 | A/B 测试增强 | 中 | VPS 实盘跑 14 天+ A/B 组数据 | ~3月8日后 |
| P2 | 12.6 | 实盘 vs 回放对比 | 中 | 12.5 VPS daemon 积累 30 天 | ~3月24日后 |
| P2 | 12.7 | 按币种差异化策略 | 中 | 按币种 ≥15 笔 (实盘或回放) | 12.4 或 12.6 后 |
| P3 | 9.2 | 信号评分模型 (LightGBM) | **高** | 9.1 特征管道 + 标注数据 | 9.1 完成后 |
| P3 | 9.3 | ML 信号过滤层 | 中 | 9.2 模型训练完成 | 9.2 后 |
| P3 | 9.4 | 特征重要性反馈 | 低 | 9.2 模型训练完成 | 9.2 后 |
| P3 | 9.5 | 模型定期重训 | 中 | 9.2 运行稳定后 | 9.2 后 |
| P4 | 11.8 | 多因子相关性分析 (lead-lag) | **高** | 9.1 特征矩阵 + 长周期数据 | 9.1 后 |
| P4 | 11.11 | Regime 概率模型 (HMM) | **高** | 足够长的 regime 历史序列 | 长期 |

#### C. 时间线总览

```
2月22日 ─── 今天 ─────────────────────────────────────────────────
  │  12.5 VPS daemon 已启动（后台持续积累）
  │
  ├── 本周 (2/22 - 2/28)
  │   ├── 12.4 多周期回放 180/365 天        ← 跑一晚上即可
  │   └── 10.3 置信区间 bootstrap           ← 1-2 天代码量
  │
  ├── 第 2 周 (3/1 - 3/7)
  │   ├── 8.2 Walk-forward 验证             ← 依赖 12.4 数据
  │   ├── 10.5 Regime 感知评估              ← 依赖 12.4 数据
  │   └── 8.5 Edge 仪表盘                   ← 可视化成果
  │
  ├── 第 3 周 (3/8 - 3/14)
